...
19. **Agent workspace.** Implemented the first read-only workspace at `/agent`: period scope selection, session-persisted conversation, tool-call trace with linked domain surfaces, source/result inspection, financial revision labels, and budget-plan preview. Contextual entry points and durable server-side conversations remain pending.
