...
- Recovery reconciliation responses now declare the persisted `recovered` session status in the Fastify/OpenAPI schema, preventing a successful bridge from failing during response serialization.
...
### `8cc72ab` — Periods query migration
...
### `52c497a` — split-bill lookup migration
...
### `1da021e` — category and tag mutation hooks

- Category create/edit/archive/restore and tag create/edit/delete now use feature-owned typed mutations with the shared financial-summary invalidation policy.
...
### `2f04cce` — CSV import contract and cache verification

- CSV import preview/confirm now use the registered `/api/transactions/import-preview` and `/api/transactions/import-confirm` paths through the generated client. The legacy UI payload is normalized at the route boundary, so account/category/period mappings, notes, references, income/expense type, payroll-period inference, and category allocations survive the import.
- Import validation is atomic and finance-aware: all referenced accounts/categories/periods are checked, closed periods are rejected, category reporting accounts are honored, and cash-equivalent lines retain explicit operating classification. DD/MM/YYYY dates are parsed before the runtime's locale-dependent slash-date parser, with invalid calendar dates rejected.
- Import preview edits now recompute row validity and summary counts. Mapping a previously missing account can make a row ready for import instead of leaving a stale error flag and disabled confirmation button.
- Added cache invalidation tests and fixed the transaction route test harness to use the same Zod validator/serializer compilers as production.

### `8614ace` — simple transaction form state discipline

- The conditional simple transaction flow now stores server-facing fields in React Hook Form with a typed Zod resolver, reset defaults, and submit-time validation. Transfer, income, expense, PayLater, route-template, transport, and tag updates all use the same typed form snapshot.
- Map pickers, attachment previews, installment previews, recommendation loading, and other transient UI concerns remain local state rather than being forced into a global store. This leaves the form boundary explicit without introducing a second server-data cache.
...
- Focused cache, transaction-mutation, and transaction-route suites pass 24 tests, including constant-query-count coverage and the CSV import invalidation boundary. Route-contract verification reports 146 paths, 188 operations, and 187 application operation IDs (the sole exception is the OAuth redirect generated by `@fastify/oauth2`).
...
9. **RHF/Zod migration is complete for the transaction modal's server-facing forms.** Simple transactions (including conditional transfer, PayLater, transport, and route-template fields), journals, edit-details, and dynamic journal lines/category allocations use typed form boundaries. Remaining local state is intentionally UI-only (pickers, previews, attachments, loading, and transient recommendations).
10. **Feature-owned React Query reads and mutations are now the default across the finance-critical pilot.** Accounts, Dashboard, Budget, Periods, Transactions (including pending approvals and route templates), Categories/Tags, Reports, Reconciliation, Agent, Subscriptions, PayLater, Salary, Anomalies, Audit Log, Split Bill, Wishlist, and loan/contact workflows now use feature query/mutation boundaries. Remaining direct calls are concentrated in domain-specific modal reads (loan/contact/account lookups, split-bill parsing), salary previews/subscription corrections, exports, and agent streaming rather than duplicate server caches.
