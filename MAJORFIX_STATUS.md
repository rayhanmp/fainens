...
10. **Feature-owned React Query reads and mutations are now the default across the finance-critical pilot.** Accounts, Dashboard, Budget, Periods, Transactions, Categories/Tags, Reports, Reconciliation, Agent, Subscriptions, PayLater, Salary, Anomalies, Audit Log, Split Bill, Wishlist, and loan deletion now use feature query/mutation boundaries. Remaining direct calls are concentrated in domain-specific modal workflows (loan creation/payment, split-bill parsing, salary/subscription corrections), exports, and agent streaming rather than duplicate server caches.
