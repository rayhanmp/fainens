    ...
    <RequireAuth>
      <PageContainer>
        ...
        <section className="mt-6 grid grid-cols-1 gap-4 md:grid-cols-2 xl:grid-cols-4">
          ...
          <article className="rounded-3xl border border-[var(--color-border)] bg-[var(--ref-surface-container-lowest)] p-6 shadow-sm"><CalendarClock className="h-5 w-5 text-[var(--ref-primary)]" /><p className="mt-4 text-xs font-bold uppercase tracking-widest text-[var(--ref-outline)]">Cash runway</p><p className="mt-2 font-headline text-3xl font-extrabold tracking-tight text-[var(--ref-on-surface)]">{analytics?.runway.isUnbounded ? 'No burn' : analytics?.runway.runwayMonths != null ? `${analytics.runway.runwayMonths} mo` : '—'}</p><p className="mt-3 text-xs text-[var(--ref-on-surface-variant)]">Based on liquid cash and recorded monthly burn</p></article>
        ...
        <section className="mt-6 grid grid-cols-1 gap-5 xl:grid-cols-3">
          <div className="rounded-3xl border border-[var(--color-border)] bg-[var(--ref-surface-container-lowest)] p-6 shadow-sm xl:col-span-2"><div className="flex flex-col gap-4 sm:flex-row sm:items-start sm:justify-between"><div><p className="text-xs font-bold uppercase tracking-widest text-[var(--ref-outline)]">Attention required</p><h2 className="mt-1 font-headline text-xl font-extrabold text-[var(--ref-on-surface)]">What deserves a decision</h2></div><span className="rounded-full bg-[var(--ref-surface-container-high)] px-3 py-1.5 text-xs font-bold text-[var(--ref-on-surface-variant)]">{attentionItems.length} item{attentionItems.length === 1 ? '' : 's'}</span></div><div className="mt-5 space-y-3">{attentionItems.length === 0 ? <div className="flex items-center gap-3 rounded-2xl bg-emerald-500/10 p-4 text-sm text-emerald-800 dark:text-emerald-200"><CheckCircle2 className="h-5 w-5 shrink-0" />No immediate issues surfaced from your recorded data.</div> : attentionItems.map((item) => <Link key={item.id} to={item.to} className="group flex flex-col gap-3 rounded-2xl border border-[var(--color-border)] p-4 transition-colors hover:bg-[var(--ref-surface-container-low)] sm:flex-row sm:items-center"><div className={cn('flex h-10 w-10 shrink-0 items-center justify-center rounded-full', item.tone === 'danger' ? 'bg-rose-500/10 text-rose-600' : item.tone === 'warning' ? 'bg-amber-500/10 text-amber-600' : 'bg-sky-500/10 text-sky-600')}><AlertTriangle className="h-5 w-5" /></div><div className="min-w-0 flex-1"><p className="font-bold text-[var(--ref-on-surface)]">{item.title}</p><p className="mt-1 text-sm text-[var(--ref-on-surface-variant)]">{item.detail}</p></div><span className="inline-flex shrink-0 items-center gap-1 text-sm font-bold text-[var(--ref-primary)]">{item.action}<ChevronRight className="h-4 w-4 transition-transform group-hover:translate-x-0.5" /></span></Link>)}</div></div>
